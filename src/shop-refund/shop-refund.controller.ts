...
export class ShopRefundController {
    ...
    @Post()
    async createRefund(@Body() refundDto: Product_RuckgabeDto): Promise<ProduktRueckgabe> {
    ...
    @Put(':id')
    async updateRefund(@Param('id') id: number, @Body() refundDto: Product_RuckgabeDto): Promise<ProduktRueckgabe> {
