...
export class SubsController {
   ...
   @UseGuards(JwtAuthGuard)
   async getEbayTopics(@Param('limit') limit: number, @Param('token') token: string) {
    try {
      await this.ebayServ.checkAccessToken();

      const topics :EbayTopicsPayloadDto = token !== 'null' ? await this.request.getRequest(`${env.ebay_api}/commerce/notification/v1/topic?limit=${limit}&continuation_token=${token}`, this.ebayServ.currentToken.access_token) : 
      await this.request.getRequest(`${env.ebay_api}/commerce/notification/v1/topic?limit=${limit}`, this.ebayServ.currentToken.access_token);
   ...
   async getEbayTopic(@Param('id') id: string) {
    try {
      await this.ebayServ.checkAccessToken();
      return await this.request.getRequest(`${env.ebay_api}/commerce/notification/v1/topic/${id}`, this.ebayServ.currentToken.access_token);
    } catch (err) {
      return err;
    }
   }
